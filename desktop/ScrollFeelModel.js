...
// A single hl.config statement carrying all four patch options. Returns ""
// for out-of-range profiles so callers never emit a broken literal.
function luaConfigStatement(profile, speed, max, decel) {
  var p = clampProfile(profile)
  if (p !== Math.round(Number(profile))) return ""
  return "hl.config({ input = { touchpad = {"
...
function parseLiveValues(lines) {
  var values = { profile: 2, speed: 1.0, max: 3.0, decel: 600 }
